...
object DimTcmsFeeType {
  ...
  private val gc_tcms = "SELECT \n" +
    ...
    s"FROM (SELECT * FROM ods.gc_tcms_xtd_customer_feekind WHERE day = '$nowDate') AS ft \n" +
    s"LEFT JOIN  (SELECT * FROM ods.gc_tcms_xtd_customer_feekind_group WHERE day = '$nowDate') AS ftg \n" +
