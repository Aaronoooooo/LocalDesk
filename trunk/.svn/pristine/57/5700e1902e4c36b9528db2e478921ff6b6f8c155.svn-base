...
object DW_fact_tcms_business_checkout_cargovolume_full {
    //任务名称(一般同类名)
    private val task = "DW_fact_tcms_business_checkout_cargovolume_full"

    //dw层类名
    private val tableName = "fact_tcms_business_checkout_cargovolume"

    // 获取当天的时间
    private val nowDate: String = DateUtil.getNowTime()

    //要执行的sql语句
    private val select_fact_tcms_business_checkout_cargovolume =
        "select bcc.row_wid, " +
          ...
          "bcc.outvolume_id," +
          "bcc.bs_id," +
          "bcc.outvolume_length," +
          "bcc.outvolume_width," +
          "bcc.outvolume_height," +
          "bcc.outvolume_grossweight," +
          "bcc.outvolume_volumeweight," +
          "bcc.outvolume_chargeweight," +
          "bcc.child_forecast_number," +
          "bcc.child_track_number," +
          "bcc.last_update_time,"+
          "date_format(bcc.w_insert_dt,'yyyyMM') as month" +
          " from " +
          " (select * from ods.gc_tcms_bsn_checkout_cargovolume where data_flag <> 'delete') as bcc"
    ...
    def main(args: Array[String]): Unit = {
        ...
        Dw_fact_common.getRunCode_hive_full_Into(task,tableName, Array(select_fact_tcms_business_checkout_cargovolume))
    }
}
