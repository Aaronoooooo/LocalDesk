...
object ReplyDelayAnalysisTest4 {
  def main(args: Array[String]): Unit = {
    val logger: Logger = LoggerFactory.getLogger(ReplyDelayAnalysisTest4.getClass)
...
java.lang.IllegalArgumentException: requirement failed: No output operations registered, so nothing to execute*/
    kafkaDStream.mapPartitions{jsonIter => {
      val jsonList: List[JSONObject] = jsonIter.map(
        record => {
      ...
      for (jsonObj <- jsonList) {
        ...
        if (usId.nonEmpty) { //用户发送消息存入redis
          ...
          if (jedis.exists(key)) {
            ...
            //按条件存入mysql
            println("客服回复消息:" + messageBody + "存入mysql")
    ...
    //定时任务执行时间
    val ExecuteTime = dayFormat.format(new java.util.Date()) + " " + "10:53:15"
