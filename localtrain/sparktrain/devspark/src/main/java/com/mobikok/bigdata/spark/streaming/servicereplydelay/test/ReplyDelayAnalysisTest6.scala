...
object ReplyDelayAnalysisTest6 {
  def main(args: Array[String]): Unit = {
    val logger: Logger = LoggerFactory.getLogger(ReplyDelayAnalysisTest6.getClass)
      ...
      rdd.foreachPartition {
        jsonObjData => {
          ...
          for (jsonObj <- jsonList) {
            ...
            //**********统一聊天窗口,pageId和senderId相等
//            val recipient = messageBody.recipient//接收者
//            val suId = messageBody.userId//客服发送
//            val pageId = messageBody.pageId//页面id
...
//            sendTime = messageBody.timestamp//发送时间
            ...
            if (usId.nonEmpty) { //用户发送消息存入redis
              ...
              filteredList+=sendTime+"_"+chatService
...
//              val key = dateState + "_" + suId + "_" + user + "_" + chatService
              val key = dateState + "_" + serviceTime(1) + "_" + user + "_" + chatService
    ...
    //定时任务执行时间
    val ExecuteTime = dayFormat.format(new java.util.Date()) + " " + "18:02:15"
